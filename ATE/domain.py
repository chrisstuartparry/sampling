...
class Domain:
    ...
    def __init__(self):
        ...
        FWAM = DiscreteParameter("firstwall_armour_material", "FW armour", ["tungsten"])
        ...
        BLT = ContinuousParameter("blanket_thickness", "Blanket thickness", 0, 500)
        BSM = DiscreteParameter(
            "blanket_structural_material", "Blanket structure", ["SiC", "eurofer"]
        )
        BBM = DiscreteParameter(
            "blanket_breeder_material", "Blanket breeder", ["Li4SiO4", "Li2TiO3"]
        )
        ...
        BBEF = ContinuousParameter(
            ...
            "Blanket breeder Li6 enrichment",
            0,
            1,
        )
        ...
        BFS = SumParameterGroup(
            "blanket_fractions",
            "Blanket fractions",
            [
                ("blanket_multiplier_fraction", "Blanket multiplier %"),
                ("blanket_breeder_fraction", "Blanket breeder %"),
                ("blanket_structural_fraction", "Blanket structure %"),
                ("blanket_coolant_fraction", "Blanket coolant %"),
            ],
            1,
        )

        FWFS = SumParameterGroup(
            "firstwall_fractions",
            "FW fractions",
            [
                ("firstwall_armour_fraction", "FW armour %"),
                ("firstwall_structural_fraction", "FW structure %"),
                ("firstwall_coolant_fraction", "FW coolant %"),
            ],
            1,
        )
    ...
    def create_data_frame(self):
        ...
        """
        return pd.DataFrame(columns=[param.name for param in self.params])
    ...
    def gen_data_frame(self, strategy, num):
        ...
        for param in self.params:
            data |= self.gen_param_values(param, strategy, num)
